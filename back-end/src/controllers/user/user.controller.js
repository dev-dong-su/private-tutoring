...
const createUser = async (req, res) => {
  ...
  if (!name || !email || !password || !userType) {
    return response(res, StatusCodes.BAD_REQUEST, false, {}, 'Please Provide all information');
  }
  ...
  try {
    ...
    if (password.length < 6) {
      return response(res, StatusCodes.NOT_ACCEPTABLE, false, {}, 'Password must be minimum 6 charecter');
    }
...
const getUsers = async (req, res) => {
  ...
  try {
    ...
    return response(res, StatusCodes.OK, true, { usersCount: usersCount, users: users }, null);
  } catch (error) {
    return response(res, StatusCodes.INTERNAL_SERVER_ERROR, false, {}, error.message);
  }
};
...
const getUserDetails = async (req, res) => {
  ...
  try {
    const user = await User.findById(id).select('name email activeStatus userType mobile photo');
    if (!user) {
...
const deleteUser = async (req, res) => {
  ...
  try {
    ...
    if (!user) {
      return response(res, StatusCodes.BAD_REQUEST, false, {}, 'Could not delete!');
    }
